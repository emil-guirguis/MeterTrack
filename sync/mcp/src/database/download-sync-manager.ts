 ...
 * 
 * Requirements: 8.1, 8.2, 8.3, 8.4, 8.5, 9.1, 9.2, 9.3, 9.4, 9.5, 10.1, 10.2, 10.3, 10.4, 10.5, 11.1, 11.2, 11.3, 11.4, 11.5
 */
...
export class DownloadSyncManager {
  ...
  async syncMeterConfigurations(): Promise<MeterSyncResult> {
    ...
    try {
      ...
      const remoteMeters = await this.queryRemoteMeters();
   ...
   */
  private async queryRemoteMeters(): Promise<MeterConfiguration[]> {
  ...
  private async syncMeters(
    ...
    for (const remoteMeter of remoteMeters) {
      ...
      if (!localMeter) {
        ...
        this.logger.info(`New meter added: ${remoteMeter.id} - ${remoteMeter.name}`);
  ...
  private meterHasChanged(remoteMeter: MeterConfiguration, localMeter: MeterConfiguration): boolean {
    ...
    return (
      remoteMeter.name !== localMeter.name ||
      remoteMeter.type !== localMeter.type ||
      remoteMeter.serial_number !== localMeter.serial_number ||
      remoteMeter.device_id !== localMeter.device_id ||
      remoteMeter.location_id !== localMeter.location_id ||
  ...
  private getChangedMeterFields(remoteMeter: MeterConfiguration, localMeter: MeterConfiguration): string[] {
    ...
    if (remoteMeter.name !== localMeter.name) changedFields.push('name');
    if (remoteMeter.type !== localMeter.type) changedFields.push('type');
    if (remoteMeter.serial_number !== localMeter.serial_number) changedFields.push('serial_number');
    if (remoteMeter.device_id !== localMeter.device_id) changedFields.push('device_id');
    if (remoteMeter.location_id !== localMeter.location_id) changedFields.push('location_id');
      ...
      async () => {
        const result = await this.remotePool.query(
          `SELECT id, name, url, street, street2, city, state, zip, country, active,
                  created_at, updated_at, meter_reading_batch_count
           FROM tenant
           ORDER BY id`
        );
  ...
  private tenantHasChanged(remoteTenant: Tenant, localTenant: Tenant): boolean {
    ...
    return (
      ...
      remoteTenant.meter_reading_batch_count !== localTenant.meter_reading_batch_count
    );
  }
  ...
  private getChangedTenantFields(remoteTenant: Tenant, localTenant: Tenant): string[] {
    ...
    if (remoteTenant.meter_reading_batch_count !== localTenant.meter_reading_batch_count) changedFields.push('meter_reading_batch_count');
    ...
    try {
      await this.localPool.query(
        ...
        [
          ...
          tenant.created_at,
          tenant.updated_at,
          tenant.meter_reading_batch_count,
        ]
      );
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
