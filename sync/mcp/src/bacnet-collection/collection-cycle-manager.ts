...
export class CollectionCycleManager {
  ...
  private async readMeterDataPoints(
    ...
    const registerMap = meter.register_map;

    // Iterate through all data points in the register map
    for (const [dataPointName, dataPointConfig] of Object.entries(registerMap)) {
      try {
        const config = dataPointConfig as any;

        // Read the property from the meter
        const readResult = await bacnetClient.readProperty(
          meter.ip,
        ...
        if (readResult.success && readResult.value !== undefined) {
          ...
          this.logger.debug(
            `Read ${dataPointName} from meter ${meter.id}: ${readResult.value} ${readResult.unit || ''}`
          );
        } else {
          // Log read error
          const errorMsg = readResult.error || 'Unknown read error';
          this.logger.warn(
            `Failed to read ${dataPointName} from meter ${meter.id}: ${errorMsg}`
          );
          errors.push({
            meterId: meter.id,
        ...
        this.logger.error(
          `Exception reading ${dataPointName} from meter ${meter.id}: ${errorMsg}`
        );
        errors.push({
          meterId: meter.id,
          dataPoint: dataPointName,
        ...
        // Continue reading other data points
      }
    }
