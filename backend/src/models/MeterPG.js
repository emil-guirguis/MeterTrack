...
class Meter {
    constructor(meterData = {}) {
        ...
        this.manufacturer = meterData.manufacturer;
        this.model = meterData.model;
    ...
    static async findById(id) {
        const query = 'SELECT * FROM meters WHERE id = $1';
    ...
    static async findAll(filters = {}) {
        ...
        if (filters.location_building) {
            paramCount++;
            query += ` AND location_building = $${paramCount}`;
            values.push(filters.location_building);
        }

        if (filters.search) {
            paramCount++;
            query += ` AND (meterid ILIKE $${paramCount} OR name ILIKE $${paramCount} OR manufacturer ILIKE $${paramCount})`;
            values.push(`%${filters.search}%`);
        }

        query += ' ORDER BY meterid ASC';
