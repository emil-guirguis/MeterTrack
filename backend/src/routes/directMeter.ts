 ...
 */
router.post('/direct-meter-read', async (req: express.Request<{}, DirectMeterResponse, DirectMeterRequest>, res: express.Response<DirectMeterResponse>) => {
  ...
  try {
    ...
    if (!connectionTest) {
      throw new ModbusError(
        `Failed to connect to meter at ${ip}:${port}`,
