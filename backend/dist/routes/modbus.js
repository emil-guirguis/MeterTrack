...
router.post('/read-meter', requirePermission('meter:read'), async (req, res) => {
    try {
        const { deviceIP, port = 502, slaveId = 1, meterType = 'generic', customRegisters } = req.body;
        if (!deviceIP) {
            return res.status(400).json({
                success: false,
                message: 'Device IP address is required'
...
router.post('/read-registers', requirePermission('meter:read'), async (req, res) => {
    try {
        const { deviceIP, startAddress, count, registerType = 'holding', // holding or input
        port = 502, slaveId = 1 } = req.body;
