...
router.post('/send', requirePermission('email:send'), [
...
router.post('/send-raw', requirePermission('email:send'), [
...
router.post('/send-bulk', requirePermission('email:send'), [
...
router.get('/delivery-stats', requirePermission('email:read'), [
...
router.get('/logs', requirePermission('email:read'), [
...
router.post('/notifications/trigger', requirePermission('notification:send'), [
...
router.get('/notifications/status', requirePermission('notification:read'), async (req, res) => {
    try {
...
router.get('/notifications/logs', requirePermission('notification:read'), [
    query('page').optional().isInt({ min: 1 }).withMessage('Page must be a positive integer'),
    query('limit').optional().isInt({ min: 1, max: 100 }).withMessage('Limit must be between 1 and 100'),
    ...
    try {
      ...
      `;
        ...
        // Get total count
        const countQuery = query.replace('SELECT nl.*, et.name as template_name, m.id as meter_name, b.name as location_name', 'SELECT COUNT(*)');
        const countResult = await db.query(countQuery, values);
        const total = parseInt(countResult.rows[0].count);
        // Add pagination
