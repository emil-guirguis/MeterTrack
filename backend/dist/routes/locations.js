...
router.get('/', [
    ...
    try {
        ...
        const { page = 1, pageSize = 20, sortBy = 'name', sortOrder = 'asc', search, 'filter.type': filterType, 'filter.status': filterStatus, 'filter.city': filterCity } = req.query;
...
router.put('/:id', [
    ...
    try {
        ...
        if (payload.totalFloors !== undefined)
            updates.totalfloors = payload.totalFloors;
        if (payload.totalUnits !== undefined)
            updates.totalunits = payload.totalUnits;
        if (payload.yearBuilt !== undefined)
            updates.yearbuilt = payload.yearBuilt;
        if (payload.squareFootage !== undefined)
            updates.squarefootage = payload.squareFootage;
        ...
        if (payload.address) {
            if (payload.address.street !== undefined)
                updates.address_street = payload.address.street;
            if (payload.address.city !== undefined)
                updates.address_city = payload.address.city;
            if (payload.address.state !== undefined)
                updates.address_state = payload.address.state;
            if (payload.address.zipCode !== undefined)
                updates.address_zip_code = payload.address.zipCode;
            if (payload.address.country !== undefined)
                updates.address_country = payload.address.country;
        }
        if (payload.contactInfo) {
            if (payload.contactInfo.primaryContact !== undefined)
                updates.contact_primarycontact = payload.contactInfo.primaryContact;
            if (payload.contactInfo.email !== undefined)
                updates.contact_email = payload.contactInfo.email;
            if (payload.contactInfo.phone !== undefined)
                updates.contact_phone = payload.contactInfo.phone;
            if (payload.contactInfo.website !== undefined)
                updates.contact_website = payload.contactInfo.website;
        }
