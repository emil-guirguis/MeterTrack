...
router.get('/', [
    ...
    try {
        ...
        const { page = 1, pageSize = 20, sortBy = 'createdAt', sortOrder = 'desc', search, 'filter.brand': filterBrand, 'filter.status': filterStatus } = req.query;
        ...
        if (!resolvedDeviceId && (req.body.brand || req.body.model)) {
            ...
            if (deviceName) {
                try {
                    ...
                    let existingDevice = allDevices.find(device => device.name === deviceName &&
