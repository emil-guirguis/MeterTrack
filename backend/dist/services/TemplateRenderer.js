...
class TemplateRenderer {
    ...
    static async renderTemplate(templateId, variables = {}, options = {}) {
        try {
            ...
            const variableResult = VariableProcessor.processVariables(variables, template.variables || [], variableProcessingOptions);
            if (!variableResult.success && options.strict) {
                throw new Error(`Variable processing failed: ${variableResult.errors.join(', ')}`);
            }
            // Render template content
            const renderResult = this.renderContent(template.subject, template.content, template.variables, variableResult.variables, options);
    ...
    static renderContent(subject, content, templateVariables = [], variables = {}, options = {}) {
        const { escapeHtml = true, allowMissingVariables = true, defaultValues = {} } = options;
        const warnings = [];
        const processedVariables = {};
        // Process subject
        const renderedSubject = this.processTemplateString(subject, templateVariables, variables, { escapeHtml, allowMissingVariables, defaultValues, warnings, processedVariables });
        // Process content
        const renderedContent = this.processTemplateString(content, templateVariables, variables, { escapeHtml, allowMissingVariables, defaultValues, warnings, processedVariables });
        return {
    ...
    static processEachBlocks(templateString, variables, context) {
        ...
        return templateString.replace(eachRegex, (match, arrayName, blockContent) => {
            ...
            array.forEach((item, index) => {
                ...
                processedBlock = this.processTemplateString(processedBlock, templateVariables, iterationContext, context);
    ...
    static getVariableValue(variablePath, variables) {
        ...
        if (variablePath === 'true')
            return true;
        if (variablePath === 'false')
            return false;
    ...
    static async previewTemplate(templateId, sampleData = {}) {
        try {
            ...
            const renderResult = this.renderContent(template.subject, template.content, template.variables, variables, { allowMissingVariables: true });
            return {
                success: true,
                data: {
                    id: template.id,
