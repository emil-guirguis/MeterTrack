...
export class ThreadingService extends EventEmitter {
     ...
     */
    async restartWorker(reason = 'Manual restart', config) {
        try {
    ...
    async sendMessage(options) {
        ...
        try {
            ...
            await this.errorHandler.handleError(error instanceof Error ? error : new Error('Unknown message error'), { source: 'message_sending', messageType: options.type });
            throw error;
        }
    }
    /**
     ...
     */
    async updateConfig(config, section) {
        const result = section
            ? this.configManager.updateSectionConfig(section, config[section])
     ...
     */
    async clearPendingMessages(reason) {
    ...
    initializeComponents() {
        ...
        this.restartManager = new RestartManager(this.threadManager, this.healthMonitor, config.restartManager);
...
//# sourceMappingURL=ThreadingService.js.map