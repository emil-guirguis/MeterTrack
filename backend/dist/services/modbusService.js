...
export class ModbusService {
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                const timeout = setTimeout(() => {
                    reject(new ModbusError('Connection timeout', ModbusErrorType.TIMEOUT, clientKey));
                ...
                socket.once('error', (error) => {
                    clearTimeout(timeout);
                    reject(new ModbusError(`Connection failed: ${error.message}`, ModbusErrorType.CONNECTION_FAILED, clientKey));
                });
            });
            const modbusClient = {
                client,
            ...
            throw new ModbusError(`Failed to connect to Modbus device at ${deviceIP}:${port} - ${error instanceof Error ? error.message : String(error)}`, ModbusErrorType.CONNECTION_FAILED, clientKey);
        }
    }
    /**
    ...
    async readMeterData(deviceIP, config = {}) {
        const { port = 502, unitId = 1, registers = this.getDefaultRegisterMapping() } = config;
        let client;
        try {
