 ...
 */
export class EnhancedModbusClient extends EventEmitter implements ModbusClientInterface {
  private client: any; // jsmodbus client
  private socket: any; // TCP socket
  private config: ModbusClientConfig;
  private logger: winston.Logger;
  private isConnected: boolean = false;
  private reconnectTimeout: NodeJS.Timeout | null = null;
  private connectionAttempts: number = 0;
  private fieldMap: FieldMapping[] | null = null;
  private lastConnectionTime: number = 0;
  private errorHandler: ModbusErrorHandler;
  ...
  private setupEventHandlers(socket: any): void {
    ...
    socket.on('timeout', () => {
      ...
      const timeoutError = new ModbusError(
        'Connection timeout',
  ...
  private loadFieldMap(): void {
    try {
      ...
      if (Array.isArray(parsed?.fields)) {
        this.fieldMap = parsed.fields as FieldMapping[];
  ...
  public async connect(): Promise<boolean> {
    ...
    try {
      await this.errorHandler.handleError(
        new Error('Initial connection attempt'),
        deviceId,
        'connect',
        async () => {
  ...
  private scheduleReconnect(): void {
    if (this.reconnectTimeout || this.isConnected) return;
  ...
  public async readMeterData(): Promise<MeterReading | null> {
    ...
    try {
      return await this.errorHandler.handleError(
        new Error('Initial read attempt'),
        deviceId,
        'readMeterData',
        async () => {
  ...
  private async readHoldingRegistersWithRetry(address: number, length: number): Promise<number[]> {
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new ModbusError(
        `Failed to read holding registers at ${address}: ${message}`,
  ...
  private async readInputRegistersWithRetry(address: number, length: number): Promise<number[]> {
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new ModbusError(
        `Failed to read input registers at ${address}: ${message}`,
        ModbusErrorType.PROTOCOL_ERROR,
        this.getDeviceId(),
        address
      );
  ...
  public getConfig(): ModbusClientConfig {
    return { ...this.config };
  }
  ...
  public updateConfig(newConfig: Partial<ModbusClientConfig>): void {
  ...
  public canReuse(config: ModbusClientConfig): boolean {
