...
export const MeterForm: React.FC<MeterFormProps> = ({
  ...
  const renderField = (fieldName: string, fieldDef: any) => {
    ...
    // Textarea for notes/description field
  ...
  const basicInfoFields = ['meterId', 'name', 'serial_number', 'type', 'status'];
  const deviceFields = ['device_id', 'device', 'model'];
  const networkFields = ['ip', 'port', 'slave_id'];
  const locationFields = ['location_id', 'location'];
  const additionalFields = ['description', 'notes', 'installation_date', 'register_map'];
  ...
  return (
    <form onSubmit={handleSubmit} className="meter-form base-form">
      ...
      <div className="meter-form__section">
        <h3 className="meter-form__section-title">Device Configuration</h3>
        
        {deviceFields.map(fieldName => {
          const fieldDef = schema.formFields[fieldName];
          return fieldDef ? renderField(fieldName, fieldDef) : null;
        })}
      </div>
      ...
      <div className="meter-form__section">
        <h3 className="meter-form__section-title">Network Settings</h3>
        
        {networkFields.map(fieldName => {
          const fieldDef = schema.formFields[fieldName];
          return fieldDef ? renderField(fieldName, fieldDef) : null;
        })}
      </div>
      ...
      <div className="meter-form__section">
        <h3 className="meter-form__section-title">Location</h3>
        
        {locationFields.map(fieldName => {
          const fieldDef = schema.formFields[fieldName];
          return fieldDef ? renderField(fieldName, fieldDef) : null;
        })}
      </div>
