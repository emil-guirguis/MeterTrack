...
export const LocationForm: React.FC<LocationFormProps> = ({
  ...
  const { schema, loading: schemaLoading, error: schemaError } = useSchema('location');
  ...
  const form = useEntityFormWithStore<Location, any>({
    ...
    entityToFormData: (locationData) => {
      ...
      return formSchema.fromApi(locationData);
    },
    getDefaultFormData: () => {
      if (!schema) return {};
      const formSchema = createFormSchema(schema.formFields);
    ...
    onSuccess: async (savedEntity, mode) => {
      console.log(`[LocationForm] ${mode} successful:`, savedEntity.id);
    ...
    onError: (error, mode) => {
      console.error(`[LocationForm] ${mode} failed:`, error);
    },
  });
  ...
  const renderField = (fieldName: string, fieldDef: any) => {
    ...
    if (fieldDef.type === 'boolean') {
      return (
        <div key={fieldName} className="location-form__field">
          <label htmlFor={fieldName} className="location-form__label">
            <input
              type="checkbox"
              id={fieldName}
              checked={value || false}
              ...
              className="location-form__checkbox"
            />
            {fieldDef.label}
            {fieldDef.required && <span className="location-form__required">*</span>}
          </label>
          {error && <span className="location-form__error">{error}</span>}
          {fieldDef.description && (
            <div className="location-form__helper-text">{fieldDef.description}</div>
          )}
        </div>
      );
    }
      ...
      return (
        <div key={fieldName} className="location-form__field">
          ...
          <select
            ...
            className={`location-form__select ${error ? 'location-form__input--error' : ''}`}
            disabled={isFormDisabled}
          >
      ...
      return (
        <div key={fieldName} className="location-form__field">
          ...
          <textarea
            ...
            value={value || ''}
            onChange={(e) => handleInputChange(fieldName, e.target.value)}
            className={`location-form__textarea ${error ? 'location-form__input--error' : ''}`}
      ...
      return (
        <div key={fieldName} className="location-form__field">
          ...
          <input
            ...
            className={`location-form__input ${error ? 'location-form__input--error' : ''}`}
            placeholder={fieldDef.placeholder}
            min={fieldDef.min}
            max={fieldDef.max}
            disabled={isFormDisabled}
  ...
  if (schemaLoading) {
    return (
      <div className="location-form base-form">
  ...
  const basicInfoFields = ['name', 'type', 'status'];
  const addressFields = ['street', 'street2', 'city', 'state', 'zip', 'country'];
  const additionalFields = ['squareFootage', 'contactId', 'active', 'notes'];

  return (
    <form onSubmit={handleSubmit} className="location-form base-form">
      ...
      <div className="location-form__section">
        <h3 className="location-form__section-title">Address</h3>
      ...
      <div className="location-form__section">
        <h3 className="location-form__section-title">Additional Information</h3>
      ...
      <div className="location-form__actions">
        ...
        >
          {form.isSubmitting ? (
            <>
              <span className="location-form__spinner" />
