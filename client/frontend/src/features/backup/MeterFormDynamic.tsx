...
interface MeterFormDynamicProps {
  meter?: Meter;
...
export const MeterFormDynamic: React.FC<MeterFormDynamicProps> = ({
  meter,
  ...
  const form = useEntityFormWithStore<Meter, any>({
    ...
    onSuccess: async (savedEntity, mode) => {
      console.log(`[MeterFormDynamic] ${mode} successful:`, savedEntity.id);
    ...
    onError: (error, mode) => {
      console.error(`[MeterFormDynamic] ${mode} failed:`, error);
    },
  });
...
export default MeterFormDynamic;
