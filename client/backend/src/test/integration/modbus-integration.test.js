...
async function checkTestDeviceAvailability() {
  try {
    const isAvailable = await modbusService.testConnection(
      process.env.TEST_MODBUS_IP || '192.168.1.100',
      parseInt(process.env.TEST_MODBUS_PORT) || 502,
      parseInt(process.env.TEST_MODBUS_SLAVE_ID) || 1
    );
